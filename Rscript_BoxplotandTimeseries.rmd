...
#  g<-g+scale_x_date(labels=date_format("%Y-%m"))#当面日付タイプに係らず年-月とする
  ...
  #消費税パート
  if(tmp.timeseries[1,1]<=as.Date("1989-04-01")){
    g<-g+geom_vline(xintercept = as.numeric(as.Date("1989-04-01")),linetype=1,colour="red",size=2)
    g<-g+geom_text(aes(x=as.Date("1989-04-01"), y=value.y*0.9, label="1989年4月-消費税導入(3%)"), size=8,family="Meiryo") 
  }
  if(tmp.timeseries[1,1]<=as.Date("1997-04-01")){
    g<-g+geom_vline(xintercept = as.numeric(as.Date("1997-04-01")),linetype=1,colour="red",size=2)
    g<-g+geom_text(aes(x=as.Date("1997-04-01"), y=value.y*0.9, label="1997年4月-消費税増税(3%→5%)"), size=8,family="Meiryo") 
  }
  if(tmp.timeseries[1,1]<=as.Date("2014-04-01")){
    g<-g+geom_vline(xintercept = as.numeric(as.Date("2014-04-01")),linetype=1,colour="red",size=2)
    g<-g+geom_text(aes(x=as.Date("2014-04-01"), y=value.y*0.9, label="2014年4月-消費税増税(5%→8%)"), size=8,family="Meiryo") 
  }  
  #消費税パート
  print(g)
  dev.off()
