...
fun.readData<-function(){
  ...
  monthly<<-2 #日次
  if(length(unique(day(dataset[,1])))==1){#月次データの日付は月中に変更
...
funMaintitle<-function(){
  ...
  mainTitle<<-mainTitle
  colnames(tmp.dataset)[2]<-mainTitle
...
dataSource<-"\nデータ出所:経済産業省"
dataSource<-"\nデータ出所:日本銀行"
dataSource<-"\nデータ出所:Federal Reserve Bank of St. Louis,株式会社東京証券取引所"
dataSource<-"\nデータ出所:内閣府"
...
dataset<-tail(dataset,datasetTail)
...
cat(paste("<hr><div style=\"text-align:center\"><b>",colnames(tmp.dataset)[2]," - 一次差分</b></div>\n",sep=""),file=htmlFile,append=T)
...
cat("<style>#chartdiv{width:100%;height:500px;font-size:11px;}</style>",file=htmlFile,append=T)			
cat("<div id=\"chartdiv\"></div>\n",file=htmlFile,append=T)  
cat("<script>\n",file=htmlFile,append=T)
cat("var chart = AmCharts.makeChart(\"chartdiv\",{\n",file=htmlFile,append=T)
cat("\"type\": \"serial\",\n",file=htmlFile,append=T)
cat("\"theme\": \"light\",\n",file=htmlFile,append=T)
cat("\"dataProvider\": [\n",file=htmlFile,append=T)
#data part  
amchartsData<-data.frame(tmp.dataset[-1,1],round(diff(tmp.dataset[,2]),10))
for(rrr in 1:nrow(amchartsData)){
  if(0<=amchartsData[rrr,2]){colType<-"#4169E1"}else{colType<-"#FF0000"}
  if(rrr!=nrow(amchartsData)){  
    cat(paste("{\"Date\": \"",format(amchartsData[rrr,1],dateFormat),"\",\"Ratio\":", amchartsData[rrr,2],",\"color\":\"",colType,"\"},\n",sep=""),file=htmlFile,append=T)
  }else{
    cat(paste("{\"Date\": \"",format(amchartsData[rrr,1],dateFormat),"\",\"Ratio\":", amchartsData[rrr,2],",\"color\":\"",colType,"\"}\n",sep=""),file=htmlFile,append=T)
  }
}
#data part    
cat("],\n",file=htmlFile,append=T)
cat("\"valueAxes\":[{\"axisAlpha\": 0.2,\"gridColor\":\"#808080\",\"gridAlpha\":0.2,\"dashLength\":0}],\n",file=htmlFile,append=T)
cat("\"gridAboveGraphs\": true,\n",file=htmlFile,append=T)
cat("\"startDuration\": 1,\n",file=htmlFile,append=T)
cat("\"graphs\": [ {\n",file=htmlFile,append=T)
cat("\"balloonText\": \"[[category]]: <b>[[value]]</b>\",\n",file=htmlFile,append=T)
cat("\"fillAlphas\": 0.8,\n",file=htmlFile,append=T)
cat("\"lineAlpha\": 0.2,\n",file=htmlFile,append=T)
cat("\"type\": \"column\",\n",file=htmlFile,append=T)
cat("\"valueField\": \"Ratio\",\n",file=htmlFile,append=T)
cat("\"fillColorsField\": \"color\"\n",file=htmlFile,append=T)
cat("}],\n",file=htmlFile,append=T)
cat("\"chartCursor\": {\n",file=htmlFile,append=T)
cat("\"categoryBalloonEnabled\": true,\n",file=htmlFile,append=T)
cat("\"cursorAlpha\": 0.2,\n",file=htmlFile,append=T)
cat("\"zoomable\": true\n",file=htmlFile,append=T)
cat("},\n",file=htmlFile,append=T)
cat("\"categoryField\": \"Date\",\n",file=htmlFile,append=T)
cat("\"categoryAxis\": {\n",file=htmlFile,append=T)
cat("\"gridColor\":\"#808080\",\n",file=htmlFile,append=T)
cat("\"gridPosition\": \"middle\",\n",file=htmlFile,append=T)
cat("\"gridAlpha\": 0.2,\n",file=htmlFile,append=T)
cat("\"tickPosition\": \"middle\",\n",file=htmlFile,append=T)
cat("\"tickLength\": 20,\n",file=htmlFile,append=T)
...
imageData<-tmp.dataset[,1:2] 
imageData<-data.frame(imageData[-1,1],diff(imageData[,2])/imageData[-nrow(tmp.dataset),2]*100)
...
mainTitle<-paste("Heat Map\n",colnames(tmp.dataset)[2],"の前月比(%)\n(注)原系列が季節調整値、非季節調整値の如何を問わず前月比をとっています。\n色が薄い程、値が大きいことを示します。")
...
setwd(pathOutput)
first.date<-c("1937-6-4","1937-7-27","1933-3-4","1934-11-15")
...
#  g<-g+scale_x_date(labels=date_format("%Y-%m"))#当面日付タイプに係らず年-月とする
  ...
  #消費税パート
  if(tmp.timeseries[1,1]<=as.Date("1989-04-15")){
    g<-g+geom_vline(xintercept = as.numeric(as.Date("1989-04-15")),linetype=1,colour="red",size=2)
    g<-g+geom_text(aes(x=as.Date("1989-04-15"), y=value.y*0.7, label="1989年4月\n消費税導入(3%)"), size=8,family="Meiryo") 
  }
  if(tmp.timeseries[1,1]<=as.Date("1997-04-15")){
    g<-g+geom_vline(xintercept = as.numeric(as.Date("1997-04-15")),linetype=1,colour="red",size=2)
    g<-g+geom_text(aes(x=as.Date("1997-04-15"), y=value.y*0.7, label="1997年4月\n消費税増税(3%→5%)"), size=8,family="Meiryo") 
  }
  if(tmp.timeseries[1,1]<=as.Date("2014-04-15")){
    g<-g+geom_vline(xintercept = as.numeric(as.Date("2014-04-15")),linetype=1,colour="red",size=2)
    g<-g+geom_text(aes(x=as.Date("2014-04-15"), y=value.y*0.7, label="2014年4月\n消費税増税(5%→8%)"), size=8,family="Meiryo") 
  }  
  #消費税パート
  print(g)
  dev.off()
...
if(ttt==1){
  colnames(htmldataset)[1:4]<-c("Date","Value","内閣総理大臣","日銀総裁")  
}else if(ttt==3){
  colnames(htmldataset)[1:4]<-c("Date","Value","President","FRB")  
}  
col<-list()
grandData<-NULL
header<-NULL
...
cat(" - 視聴可能ウェブブラウザは http://caniuse.com/#feat=speech-recognition で確認ください。<br>\n",file=htmlFile,append=T)
...
rankList<-list()
allSentence<-""
bufcolnmaes<-colnames(tmp.dataset[,1:2])
for(eee in 1:2){
  if(eee==1){
    rankData<-tmp.dataset[,1:2]
    sentence<-"原数値について、"
  }else{  
    rankData<-data.frame(Date=tmp.dataset[-1,1],value=diff(tmp.dataset[,2]))
    colnames(rankData)<-bufcolnmaes  
    sentence<-"続いて前月差について、"
  }
  ...
  allSentence<-paste(allSentence,sentence)
}
#speech
#SpeechRecognitionAPI
#冗長に過ぎるので要改良
phrase<-gsub("、|。","<<>>",allSentence)
phrase
cat("\n\n\n\n<script>\n",file=htmlFile,append=T)
...
cat("greeting=\"こんばんは\";\n",file=htmlFile,append=T)
cat("}  \n",file=htmlFile,append=T)
cat("greeting = greeting + \"<<>>アセットマネジメントコンサルティング株式会社です<<>>\"\n",file=htmlFile,append=T)
cat(paste("phrase=\"",phrase,"\"\n",sep=""),file=htmlFile,append=T)
cat(paste("phrase = greeting + \"", colnames(tmp.dataset)[2]  ,"の\" + phrase\n",sep=""),file=htmlFile,append=T)
cat("phrase = phrase + \"以上となります<<>>ご聴取ありがとうございました\"\n",file=htmlFile,append=T)
