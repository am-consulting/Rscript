...
for(ttt in 1:2){ #ttt 1:首相 2:日銀総裁
  ...
  g<-g+annotate("text",x=tmp.timeseries[1,1],y=y1-(y1-y0)/6,label=plotText,size=10,family="Meiryo",hjust=0)
  g<-g+scale_x_date(labels=date_format(dateFormat))
  g<-g+scale_y_continuous(labels=comma,limits=c(y0,y1))
  print(g)
  dev.off() 
}
#以下はデータ確認用にグローバル化する
