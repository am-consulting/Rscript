...
```{r}
commonTitle<-"日本国債金利情報･米国財務省証券金利･10年物金利差:"
...
write.csv(na.omit(dataset),csvFile,col.names=T,row.names=F,append=F,quote=F,fileEncoding="UTF-8")
```

```{r}
fun.readData()
commonTitle<-"貸出･預金動向.速報:"
dataSource<-"\nデータ出所:日本銀行"
```
...
for(ttt in 1:2){ #ttt 1:首相 2:日銀総裁
  ST<-Sys.time()  
  addVer<-gsub("\\.","",paste(year(SD),month(SD),day(SD),hour(ST),minute(ST),second(ST),sep=""))
...
for(ttt in 1:2){    
  ...
  g<-g+ggtitle(paste(commonTitle,mainTitle,dataSource))
  g<-g+theme(axis.text.x=element_text(size=40,angle=90,hjust=1,vjust=0.5,face="bold",family="Meiryo")) 
...
switch(monthly,
timeDiff<-paste(",Difference:",(as.yearmon(as.Date(latestDate))-as.yearmon(as.Date(firstDate)))*12,"months"),  
