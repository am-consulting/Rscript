...
for(ttt in 1:2){
  ...
  if(length((tmp.timeseries$value)[tmp.timeseries$value<0])!=0){
  ...
  g<-g+theme(legend.text=element_text(colour="black",size=25,face="bold",family="Meiryo"))          
  g<-g+theme(legend.title=element_text(colour="white",size=0,face="bold")) 
  g<-g+annotate("text",x=tmp.timeseries[5,1],y=max(tmp.timeseries[,2])*0.9,label=plotText,size=7,family="Meiryo",hjust=0)
  if(monthly==1){
...
maxDate<<-as.Date(paste((subset(tmp.dataset,max(tmp.dataset[,2])==tmp.dataset[,2])[,1]),collapse=","))
if(monthly==1){maxDate<<-format(maxDate,"%Y/%m")}  
maxValue<<-unique(as.vector(subset(tmp.dataset,max(tmp.dataset[,2])==tmp.dataset[,2])[,2]))
maxPM<<-unique(as.vector(subset(tmp.dataset,max(tmp.dataset[,2])==tmp.dataset[,2])[,3]))
maxBoJ<<-unique(as.vector(subset(tmp.dataset,max(tmp.dataset[,2])==tmp.dataset[,2])[,4]))
minDate<<-as.Date(paste((subset(tmp.dataset,min(tmp.dataset[,2])==tmp.dataset[,2])[,1]),collapse=","))
if(monthly==1){minDate<<-format(minDate,"%Y/%m")}  
minValue<<-unique(as.vector(subset(tmp.dataset,min(tmp.dataset[,2])==tmp.dataset[,2])[,2]))
minPM<<-unique(as.vector(subset(tmp.dataset,min(tmp.dataset[,2])==tmp.dataset[,2])[,3]))
minBoJ<<-unique(as.vector(subset(tmp.dataset,min(tmp.dataset[,2])==tmp.dataset[,2])[,4]))
latestDate<<-as.Date(last(tmp.dataset[,1]))
if(monthly==1){latestDate<<-format(latestDate,"%Y/%m")}  
latestValue<<-last(tmp.dataset[,2])
latestPM<<-last(tmp.dataset[,3])
latestBoJ<<-last(tmp.dataset[,4])
firstDate<<-as.Date(first(tmp.dataset[,1]))  
if(monthly==1){firstDate<<-format(firstDate,"%Y/%m")}  
