...
fun.readData<-function(){
  ...
  monthly<<-2
  if(unique(day(dataset[,1]))==1){#月次データの日付は月末に変更
    dataset[,1]<-as.Date(paste(year(dataset[,1]),"-",month(dataset[,1]),"-",days_in_month(dataset[,1]),sep=""))
    monthly<<-1
  }
...
for(ttt in 1:2){
  ...
  g<-g+annotate("text",x=tmp.timeseries[5,1],y=max(tmp.timeseries[,2])*0.95,label=plotText,size=5,family="Meiryo",hjust=0)
  if(monthly==1){
