...
if(colnames(dataset)[1]=="Date"){ #1列目が日付でない場合、強制的にオーダー化する
...
```{r}
setwd(pathOutput)
overallTitle<-"民主党政権と自民党政権(第二次安倍政権以降)の比較:労働力調査(単位:万人,パーセント) データ出所：総務省統計局\n月次データの日付は次の例の通り:(例)2015年1月→2015年1月15日,2015年2月→2015年2月14日"
labels<-c("民主党政権:2009/9/16-2012/12/25","自民党政権:2012/12/26-")
labelValue<-0
changePoint<-as.Date(c("2009/9/16","2012/12/26"))
monthly<-2
buf<-dataset
if(unique(day(buf[,1]))==1){#月次データの日付は14日または15日に変更
  buf[,1]<-as.Date(paste(year(buf[,1]),"-",month(buf[,1]),"-",floor(days_in_month(buf[,1])/2),sep=""))
  monthly<-1
}
switch(monthly,
  dateFormatType<-"%Y/%m",
...
png.file<-paste(addVer,".png",sep="")
png(file=png.file,width=1400,height=900)
mfrowTYpe<-c(2,ceiling((ncol(buf)-1)/2))
par(oma=c(1,1,7,1),mfrow=mfrowTYpe,ps=20,mar=c(3,3,5,3),cex.main=1.2,cex.lab=1,cex.axis=1.0,family="Meiryo")  
for(ccc in 2:ncol(buf)){
  ylimMin<-min(subset(buf,changePoint[1]<=buf[,1])[,ccc])
  ylimMax<-max(subset(buf,changePoint[1]<=buf[,1])[,ccc])
  plot(0,0,type="n",xlim=c(1,xlimEnd),ylim=c(ylimMin,ylimMax),xlab="",ylab="",main=colnames(buf)[ccc])
  for(mmm in 1:length(data)){
    tmpDF<-data.frame(id=c(1:nrow(data[[mmm]])),data[[mmm]])
    tmp<-tmpDF[,c(1,2,ccc+1)]
    points(tmp[,1],tmp[,3],col=cols[mmm])
    lines(tmp[,1],tmp[,3],type="l",col=cols[mmm])
    if(labelValue==1){
      text(tmp[,1],tmp[,3],labels=tmp[,3],srt=0,cex=0.7,adj=0,col=cols[mmm],family="Meiryo")
    }
    text(tmp[1,1],tmp[1,3],labels=format(tmp[1,2],dateFormatType),srt=0,cex=1,adj=0,col="black",family="Meiryo")
    text(tmp[nrow(tmp),1],tmp[nrow(tmp),3],labels=format(tmp[nrow(tmp),2],dateFormatType),srt=0,cex=1,adj=1,col="black",family="Meiryo")
    abline(lm(tmp[,3]~tmp[,1]),col=cols[mmm],lty="dashed")
  }
  legend("topleft",legend=labels,col=cols,lty=ltys)
}
title(overallTitle,outer=TRUE,cex.main=1.5,family="Meiryo")
