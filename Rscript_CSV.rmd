...
```{r}
overallTitle<-"対内証券売買契約等の状況(単位:億円、株式他は株式･投資ファンド持分、データ出所:財務省)"
menulength<-20
tailNumber<-13
...
par(oma=c(5,1,5,1),mfrow=mfrowTYpe,ps=20,mar=c(10,5,5,5),cex.main=1.2,cex.lab=1.0,cex.axis=1.0)
if(colnames(dataset[1])!="Date"){
...
for(ccc in 2:ncol(dataset)){
  ...
  if(plotType==1){
    ...
    if(3<=dateFormatType){
      barplot(names.arg=format(plotX,dateFormatType),tmp[,2],horiz=F,las=2,main=mainTitle,family="Meiryo",ylab="",xlab="",col=tmp[,3],yaxt="n") 
    }else{    
