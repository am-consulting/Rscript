...
```{r}
overallTitle<-"建設技能労働者過不足率(%)の推移(全国,原数値):2015年3月(データ出所：国土交通省)"
tailNumber<-12
...
for(ccc in 2:ncol(dataset)){
  ...
  if(plotType==1){
    ...
    barplot(names.arg=format(plotX,dateFormatType),tmp[,ccc],horiz=F,las=2,main=mainTitle,family="Meiryo",ylab="",xlab="",col="blue",yaxt="n") 
