...
funPlotTimeSeries <- function() {
  ...
  lo <- loess(dataSet[, 2] ~ as.numeric(dataSet[, 1]))
  ...
  # lo <- loess(wtiOil0[, 2] ~ as.numeric(wtiOil0[, 1]))
  # lines(wtiOil0[, 1] , predict(lo) , col = 'red', lwd = 2, lty = 2)
  axis(4, cex.axis = 2.5, las = 0, padj = 0.5)
  mtext(colnames(dataSet)[2],  side = 2,  line = 3.2,  cex = 2)
  mtext(colnames(wtiOil0)[2],  side = 4,  line = 3.2,  cex = 2)
  axis.Date(side = 1, at = dataSet[, 1], format = dateFormat, padj = 1, cex.axis = 2)
  graphics::legend(
    'topleft',col=c('blue','red'),lty=1,legend=c(colnames(dataSet)[2],colnames(wtiOil0)[2]),cex=2.5,bty='n')
  dev.off()
}
```
...
colnames(tmp)[2] <- 'Crude Oil Prices: West Texas Intermediate (WTI) - Cushing, Oklahoma'
wtiOil <- tmp
```
