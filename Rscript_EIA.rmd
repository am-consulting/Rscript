...
funPlotTimeSeries <- function() {
  ...
  par(mar = c(6, 8, 13, 9), family = 'Meiryo')
  plot(
    dataSet[, 1],
    dataSet[, 2],
    type = 'h',
    ...
    cex.axis = 2.5, cex.main = 2.5, lwd = 2
  )
  ...
  mtext(colnames(dataSet)[2],  side = 2,  line = 5.2,  cex = 2)
  mtext(gsub('-','\n',colnames(dataSet)[4]),  side = 4,  line = 6.2,  cex = 2)
  axis.Date(side = 1, at = dataSet[, 1], format = dateFormat, padj = 1, cex.axis = 2)
  graphics::legend(
   'topleft',col=c('blue','red'),lty=1,legend=colnames(dataSet)[c(2,4)],cex=2.5,bty='n',lwd = 2)
  dev.off()
}
```
...
for (ooo in 1:length(obj)) {
  ...
  cat('<b>',paste0(colnames(tmp0)[2], '</b><br>'), file = htmlFile, append = T)
  cat('<b>',paste0(JapaneseTitle[ooo], '</b>\n\n'), file = htmlFile, append = T)
  colnames(tmp0)[2] <- 'Figure'
  bufTable <- tail(tmp0, 10)
  ...
  # summaryTable <- data.frame(apply(dataSet[,2:3],2,summary), check.names = F, stringsAsFactors = F)
...
# summaryTable <- data.frame(apply(dataSet[,4,drop=F],2,summary), check.names = F, stringsAsFactors = F)
# colnames(summaryTable) <- paste0(colnames(summaryTable), ':', first(dataSet[, 1]), '~', last(dataSet[, 1]))
# summaryTable <- knitr::kable(summaryTable)
# cat('\n***\n', file = htmlFile, append = T)
