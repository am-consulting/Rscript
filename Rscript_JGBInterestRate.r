...
historicalData <-
  read.csv(
    paste(jgbBaseURL, jgbFiles[1], sep = ""),
    header = T,
    skip = 1,
    stringsAsFactor = F,
    check.names = F,
...
currentData <-
  read.csv(
    paste(jgbBaseURL, jgbFiles[2], sep = ""),
    header = T,
    skip = 1,
    stringsAsFactor = F,
    check.names = F,
...
if (length(buf[buf == F]) == 0) {
  ...
  tmp[, 1] <- as.Date(tmp[, 1])
  tmp <-
    data.frame(tmp[1], apply(tmp[-1], 2, as.numeric), check.names = F)
} else {
  tmp <- "Some sort of change(s) in source data."
}
jgbData <<- tmp