...
tmp01<-na.omit(tmp)
tmp02<-na.omit(tmp)
tmp<-merge(tmp01,tmp02)
if(2<ncol(tmp)){
tmp01<-tmp[,c(1,2)]
tmp02<-tmp[,c(1,3)]
}
head(tmp);tail(tmp);tmp<- na.omit(tmp)
...
png(file=png.file,width=950,height=850)
...
dataset$color<-ifelse(dataset[,1]<=as.Date("2012/12/1"),"indianred1","lightblue")
...
g<-g+ggtitle(
  paste("民主党政権発足から現在(安倍政権)までの実質輸出入の動向 Source:日本銀行\n赤:民主党 , 青:自民党",sep=""))
g<-g+xlab(colnames(dataset)[2])+ylab(colnames(dataset)[3])
g<-g+theme(plot.title=element_text(size=16,family="Meiryo"))
...
g<-g+geom_text(data=dataset,aes(x=dataset[,2],y=dataset[,3],label=dataset$number),
  size=3.5,vjust=0,hjust=2,family="Meiryo")
...
cat(paste("<div style=\"text-align:center\"><img src=\"http://archive.am-consulting.co.jp/",png.file,"\" alt=\"\" title=\"\" width=\"100%\"></div><br>",sep=""),file=htmlFile,append=T)
cat("<b>",colnames(dataset)[2],"</b>",file=htmlFile,append=T)
...
cat("<hr><b>",colnames(dataset)[3],"</b>",file=htmlFile,append=T)
