...
```{r}
datafreq<-12 # 4 , 12
posttitle<-"民主党政権発足から現在(安倍政権)までの雇用失業率と欠員率"
datasource<-"総務省"
library(ggplot2)
library(xtable)
#library(ggrepel)
...
g<-g+geom_text(data=dataset,aes(x=dataset[1,2],y=dataset[1,3],
  ...
  size=6,vjust=1,hjust=1,family="Meiryo")
g<-g+geom_text(data=dataset,aes(x=dataset[nrow(dataset),2],y=dataset[nrow(dataset),3],
  label=format(dataset[nrow(dataset),1],"%Y-%m")),size=6,vjust=0,hjust=0,family="Meiryo")
