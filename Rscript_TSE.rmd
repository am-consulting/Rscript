```{r}
period<-c("03/16～03/20","03/23～03/27")
...
for(ppp in 1:3){
  ...
  g1<-g1+geom_text(data=balance.dataset,aes(x=investor,y=lastWeek/2,label=comma(round(lastWeek,2))),angle=90,vjust=0.5,hjust=0,size=7,family="Meiryo") 
  ...
  g1<-g1+theme(axis.text.y=element_text(size=30,family="Meiryo")) 
  ...
  g2<-g2+geom_text(data=balance.dataset,aes(x=investor,y=thisWeek/2,label=comma(round(thisWeek,2))),angle=90,vjust=0.5,hjust=0,size=7,family="Meiryo") 
  ...
  g2<-g2+theme(axis.text.y=element_text(size=30,family="Meiryo")) 
  ...
  grid.arrange(g1,g2,ncol=2,main=textGrob(mainTitle,gp=gpar(fontsize=35, fontfamily="Meiryo")))
  dev.off()
}
```