...
for(rrr in 1:length(origData)){
  ...
  tmp<-subset(tempAnomalyDF[[rrr]],(year(last(tempAnomalyDF[[rrr]][,1]))-5)<=year(tempAnomalyDF[[rrr]][,1]))
  ...
  seasonplot(tempAnomalyTS,col=rainbow_hcl(6,c=150),year.labels=TRUE,lwd="2",main=mainTitle,family="Meiryo",ylab=title[[rrr]],cex.main=1,cex.lab=1,cex.axis=1) 
  dev.off()
}
########## seasonplot
```
...
for(rrr in 1:length(origData)){
  ...
  if(level.type==2){
    g<-g+geom_bar(data=tempAnomalyDF[[rrr]],aes(x=Date,y=value),stat="identity",position="identity",fill="blue",alpha=0.2)
  }else{
    g<-g+geom_line(data=tempAnomalyDF[[rrr]],aes(x=Date,y=value),size=1,colour="blue",alpha=0.5)
  }
  g<-g+geom_smooth(data=tempAnomalyDF[[rrr]],aes(x=Date,y=value,group=1),method=loess,color="red",lwd=1)
...
for(rrr in 1:length(origData)){
  ...
  plot(result.forecast,main=paste(mainTitle,subTitle),lwd="1",xlim=c(nrow(tempAnomalyDF[[rrr]])-forecastFoward,nrow(tempAnomalyDF[[rrr]])+forecast.interval),ylim=c(ymin,ymax),cex.main=1,family="Meiryo",ylab=title[[rrr]],cex.lab=4,cex.axis=4,cex.sub=1)  
  text(x=nrow(tempAnomalyDF[[rrr]])-forecastFoward,y=ymax*0.95,caution,cex=0.8,adj=0)
  dev.off()
}  
########## arima
```