...
if(exe==1){
  ts.source<-list()
  ts.source[[1]]<-c("FRED",1,"DGS10","GOLDAMGBD228NLBM","DCOILWTICO","NIKKEI225","SP500","DJIA")
  ts.source[[1]]<-c("FRED",1,"DEXKOUS")
  #,"NASDAQ100","DTWEXM","DEXJPUS","DEXUSEU","DEXUSUK","DEXUSAL","DEXNOUS","DEXSZUS","DEXSFUS","DEXKOUS")#,"DEXCAUS","DEXCHUS","DEXINUS","DEXBZUS","DEXMXUS","DEXUSNZ","DEXTHUS","DEXTAUS","DEXSIUS","DEXMAUS","DEXSDUS","DEXHKUS","DEXDNUS","DEXSLUS" UNRATE CPIAUCSL HOUSTNSA EXJPUS 
...
###################################### 共 通 パート ######################################
fun.withkeyposition()
zzzz<-0#関数化確認用パート.動作に問題なければパートごと削除
if(zzzz==1){#関数化確認用パート
#本ループで日付、データ値、PM(USA)、BOJ(FRB)のデータフレーム作成
for(ttt in ttt.range){
  if(chk==0){
...
#  png.file<-paste(js.title[[iii]],data.id,id,ppp,iii,ttt,".png",sep="")  
  ...
  g<-g+ggtitle(gettitle)+xlab("")+ylab("")
  g<-g+theme(axis.text=element_text(size=30,color="black",angle=90,hjust=1),axis.title=element_text(size=30,face="bold"),title=element_text(size=45,face="bold")) 
  print(g)
  dev.off()
...
png.file<-paste(js.title[[iii]],"setar",ppp,iii,".png",sep="")
png(file=png.file,width=1400,height=1200)    
par(ps=30)
par(mar=c(9, 7, 15, 4))
