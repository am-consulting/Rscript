  ...
  if(plot.type==2){
    for(ddd in 1:cnt){
      ...
      if(yy.type<=nrow(tmp)){
        ...
        colnames(dataset.xts.yy[[ddd]])<-paste("YoYchange.",colnames(dataset.xts.yy[[ddd]]),sep="")
        ...
        colnames(all.data.xts.yy[[1]])<-paste("YoYchange.",colnames(all.data.xts.yy[[1]]),sep="")
...
fun.gettitle<-function(){
  ...
  if(regexpr("YoYchange",gettitle)!=-1){
    gettitle<-gsub("億円","percent",gettitle, perl=T)
  }
  gettitle<-gsub("\\.億円","",gettitle, perl=T)
  gettitle<-gsub("YoYchange.","YoYchange\n",gettitle, perl=T)
  gettitle<-gsub("\\.q\\.","",gettitle, perl=T)
  gettitle<-sub("\\.", "\n", gettitle)
  if(monthly==1){
...
colnames(tmp.table)[2]<-gsub("\\.q\\.","",colnames(tmp.table)[2], perl=T)  
#テーブルcolname修正パート
...
#unit root test graphimage
  ...
  if(regexpr("YoYchange",tmp.xts.title)!=-1){
    tmp.xts.title<-gsub("億円","percent",tmp.xts.title, perl=T)
  }
  tmp.xts.title<-gsub("\\.億円","",tmp.xts.title, perl=T)    
  tmp.xts.title<-gsub("\\.q\\.","",tmp.xts.title, perl=T)    
...
#javascript part for forecast
#future
tmp.forecast<-na.omit(tmp.1[,c(1:2)]) 
...
#javascript part
tmp.fitting<-na.omit(tmp.1[,c(1:2)])
