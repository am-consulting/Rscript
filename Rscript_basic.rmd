...
#    gettitle<-paste(format(tmp.1[1,1],"%Y-%m"),"-",format(tmp.1[nrow(tmp.1),1],"%Y-%m"),"\n",gettitle) #2016-05-12
    gettitle<-paste(format(tmp.1[1,1],"%Y-%m"),"-",format(tmp.1[nrow(tmp.1),1],"%Y-%m")," ",gettitle) #2016-05-12
  }else{
#    gettitle<-paste(tmp.1[1,1],"-",tmp.1[nrow(tmp.1),1],"\n",gettitle) #2016-05-12
    gettitle<-paste(tmp.1[1,1],"-",tmp.1[nrow(tmp.1),1]," ",gettitle) #2016-05-12
  }
  gettitle<<-gettitle
}
####################################################### gettitle         ################################################
...
tmp.0<-subset(tmp.0,border.date<=tmp.0[,1])  
if(nrow(tmp.0)!=0){
...
##################################### apply非適用パート、確認用     
##################################### apply適用パート、PMとBoJは長さの異なる個別のデータとして流用することが出来る様、最後にmergeする。
    if(sw.keyposition==2){
    for(nnn in 1:nrow(dataset.tm[[ttt]])){  
      if(ttt==1 | ttt==3){  
        subset.tmp.0<-subset(tmp.0,dataset.tm[[ttt]]$inaugural.date[nnn]<=tmp.0$Date & tmp.0$Date<=dataset.tm[[ttt]]$resignation.date[nnn])
        if(nrow(subset.tmp.0)!=0){
          subset.tmp.0$p1<-dataset.tm[[ttt]]$Name[nnn]
          buf.tmp.0<-rbind(buf.tmp.0,subset.tmp.0)    
        }
      }else{
        subset.tmp.1<-subset(tmp.0,dataset.tm[[ttt]]$inaugural.date[nnn]<=tmp.0$Date & tmp.0$Date<=dataset.tm[[ttt]]$resignation.date[nnn])
        if(nrow(subset.tmp.1)!=0){
          subset.tmp.1$p2<-dataset.tm[[ttt]]$Name[nnn]
          buf.tmp.1<-rbind(buf.tmp.1,subset.tmp.1)    
        }  
      }  
    }
    if(ttt==2 | ttt==4){  
      tmp.0<-merge(buf.tmp.0,buf.tmp.1)    
    }  
    }
##################################### apply適用パート   
...
cnt<<-0
#data.id<-c("tschart","boxplot","unitroot","acf","forecast","return","rank","garch","msm","setar","seasonplot") #2016-05-12
data.id<-c("tschart","boxplot","unitroot","acf","forecast") #2016-05-12
border.date<-as.Date(max(dataset.tm[[1]][1,1],dataset.tm[[2]][1,1]))
####################################################### argument  #######################################################
```
...
ts.source[[2]]<-c("FRED",1,"DTWEXM","DEXJPUS","DEXUSEU","DEXUSUK","DEXUSAL","DEXNOUS","DEXSZUS","DEXSFUS","DEXKOUS","DEXUSNZ","DEXCHUS","TEDRATE")
#ts.source[[3]]<-c("FRED",1,"PAYEMS","MANEMP","UNRATE","CPIAUCNS","HOUSTNSA","A191RL1Q225SBEA","AMBNS","M2V","CP0000EZ17M086NEST","EXJPUS") #2016-05-12
for(lll in 1:length(ts.source)){
  if(2<length(ts.source[[lll]])){
    for(iii in 3:length(ts.source[[lll]])){
      ...
      if(js.title[[cnt]]!="UNRATE" & js.title[[cnt]]!="A191RL1Q225SBEA" & js.title[[cnt]]!="M2V"  & lll==3){  
        plot.type[[cnt]]<-2
      }else{
        plot.type[[cnt]]<-1
      }
...
#####################################################    JGB part    ####################################################
##################################################### interest spread ###################################################
if(colnames(dataset[[current.cnt]])[2]=="DGS10"){
  end.cnt<-cnt
  cnt<<-end.cnt+1
  dataset[[cnt]]<-merge(dataset[[current.cnt]],dataset[[end.cnt]])
  dataset[[cnt]]$InterestSpread<-(dataset[[cnt]][,2]-dataset[[cnt]][,3])
  dataset[[cnt]]<-dataset[[cnt]][,-(2:3)]
  js.title[[cnt]]<-"10yearInterestSpread"
  yy.type[[cnt]]<-12
  plot.type[[cnt]]<-1
  monthly[[cnt]]<-0
}
      ...
      for(ppp in 1:plot.type[[iii]]){
        ...
        for(ttt in ttt.range){
          if(tmp0!=0){
            ...
            png(file=png.file,width=1900,height=1200) #2016-05-12  
            g<-ggplot()
            ...
            g<-g+ylim(y0,y1)
            #2016-05-12
            g<-g+theme(#"plain", "italic", "bold", "bold.italic"
              axis.text =element_text(size=70),
              axis.title=element_text(size=70,face="plain"),
              title=element_text(size=70,face="plain"),
              legend.position="right",
              legend.text =element_text(colour="black",size=70,face="plain"),
              legend.title=element_text(colour="white",size=12,face="plain")
            ) 
...
#表示は5年分とする
if(monthly[[iii]]==1){
  data.lag<-12*5#オリジナルデータの長さに注意
  tmp.table<-head(tmp.table,data.lag)
}else{
  data.lag<-as.Date(paste(year(Sys.Date())-5,"-",month(Sys.Date()),"-",day(Sys.Date()),sep=""))#データの最終日付からでは無いことに注意
  tmp.table<-subset(tmp.table,data.lag<=tmp.table[,1])  
}   
...
###################################### boxplotパート######################################
#以降で適用するデータフレーム
#2015/03/25 全データを対象とする
if(monthly[[iii]]==1){
 data.lag<-12*1000#オリジナルデータの長さに注意
 tmp.1<-tail(tmp.1,data.lag)  
}else{
 data.lag<-as.Date(paste(year(Sys.Date())-1000,"-",month(Sys.Date()),"-",day(Sys.Date()),sep=""))#データの最終日付からでは無いことに注意
...
```{r}
###################################### chart part ######################################## 
#他のパートと異なる点があるためコードを流用しないこと
setwd(pathOutput)
fun.convert()
data.id<-"Correlations"
first.date<-as.Date(paste(year(Sys.Date())-5,"-",month(Sys.Date()),"-",day(Sys.Date()),sep=""))
colnames(all.data.xts[[1]])
buf<-all.data.xts[[1]][,c(1:4,6,9,10,12,30,31)]
buf<-na.omit(buf[paste(first.date,"::",sep="")])
output.html<-paste(data.id,".html",sep="")
cat("",file=output.html,append=F)
for(ooo in 1:2){
  png.file<-paste(data.id,ooo,".png",sep="")
  png(file=png.file,width=1400,height=1200)    
  par(mar=c(7, 7, 25, 7))
  par(ps=30)
  if(ooo==1){
    buf.corr<-buf
    f.date<-first(index(buf.corr))
    e.date<-last(index(buf.corr))
    maintitle<-paste("Level.",f.date,"-",e.date,sep="")
  }else{
    buf.corr<-na.omit(diff(buf))
    f.date<-first(index(buf.corr))
    e.date<-last(index(buf.corr))
    maintitle<-paste("1st Difference.",f.date,"-",e.date,sep="")
  }
  chart.Correlation(buf.corr,method="pearson",main=maintitle,cex.main=1,histogram=F)
  dev.off()
  cat(paste("<div style=\"text-align: center\"><b>",maintitle,"</b></div>\n",sep=""),file=output.html,append=T)
  cat(paste("<img src=\"http://archive.am-consulting.co.jp/",png.file,add.ver,"\"", " alt=\"\" width=\"100%\">\n",sep=""),file=output.html,append=T)
  cat(paste("<br><hr><br>\n",sep=""),file=output.html,append=T)
}
dev.off() 
#javascript for html    
js.file.name<-paste(data.id,".external01.js",sep="")
...
cat(paste("\n$('#","corrcoef","').html('<pre>' + data + '</pre>');",sep=""),file=js.file.name,append=T)
cat("\n});",file=js.file.name,append=T)
cat("\n});\n\n",file=js.file.name,append=T)
###################################### chart part ########################################  
######################################  csv part  ########################################  
js.file.table.list<-paste(data.id,".tablelist.js",sep="")
cat("",file=js.file.table.list,append=F) 
js.file.name<-paste(data.id,".external02.js",sep="")
cat("",file=js.file.name,append=F) 
buf<-all.data.xts[[1]][,c(1:19,30:31)]
buf<-na.omit(buf[paste(first.date,"::",sep="")])
for(iii in 1:2){
  grand.result<-NULL
  if(iii==1){  
    result.cor<-round(cor(buf,method="pearson"),3)
    f.date<-first(index(buf))
    e.date<-last(index(buf))
    chart.title<-paste("Correlation:Level<br>",f.date,"-",e.date,sep="")
  }else{
    result.cor<-round(cor(na.omit(diff(buf)),method="pearson"),3)
    f.date<-first(index(na.omit(diff(buf))))
    e.date<-last(index(na.omit(diff(buf))))
    chart.title<-paste("Correlation:1st difference<br>",f.date,"-",e.date,sep="")
  }
  for(ccc in 1:ncol(result.cor)){
    temp.1<-paste(row.names(result.cor),"-",row.names(result.cor)[ccc],sep="")
    temp.df<-data.frame(temp.1[-(1:ccc)],result.cor[-(1:ccc),ccc],row.names=NULL)
    grand.result<-rbind(grand.result,temp.df)
  }
  colnames(grand.result)<-c("Pair","Corr")
  csv.file.name<-paste(data.id,iii,".csv",sep="")
  write.csv(grand.result,csv.file.name,col.names=T,row.names=F,append=F,quote=F,fileEncoding="UTF-8")
  csv.id<-paste("chartmulticorrelation",iii,sep="")
  cat(paste("document.write(\"<b>",chart.title,"</b>\");\n",sep=""),file=js.file.table.list,append=T)  
  cat(paste("document.write('<div id=\"",csv.id,"\"></div>\');\n",sep=""),file=js.file.table.list,append=T)
  ...
  cat("\n\"order\": [[ 1,\"desc\"]],",file=js.file.name,append=T)      
  cat("\n\"bFilter\":false,",file=js.file.name,append=T)
  cat("\n\"paging\":true,",file=js.file.name,append=T)
  cat("\n\"columnDefs\":[{\"width\":\"70%\",\"targets\":0}],",file=js.file.name,append=T) 
...
######################################引数設定パート######################################    
title.no<-6
explanatoryList<-c(3,3,3,3,3,46) #column Date列分1を加える
objectiveList<-c(4,46,14,49,10,49) #column Date列分1を加える
explanatory<-explanatoryList[title.no]
objective<-objectiveList[title.no]
ttt.range<-c(1:2)
...
iii<-cnt+1
######################################引数設定パート###################################### 
dataset[[iii]]<-tail(na.omit(all.data[[1]][,c(1,explanatory,objective)]),priod.y*12)
colnames(dataset[[iii]])[1]<-"Date"
fun.withkeyposition()
