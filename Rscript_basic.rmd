...
output4.html<-paste("acf",id,".html",sep="")
...
js.file.name.5<-paste(js.title,".external05.js",sep="")
data.id<-"acf"
cat("\n\n$(function() {",file=js.file.name.5,append=F)
cat(paste("\n$.get('http://archive.am-consulting.co.jp/",output4.html,add.ver,"', function(data) {",sep=""),file=js.file.name.5,append=T)
cat(paste("\n$('#",data.id,"').html('<pre>' + data + '</pre>');",sep=""),file=js.file.name.5,append=T)
cat("\n});",file=js.file.name.5,append=T)
cat("\n});\n\n",file=js.file.name.5,append=T) 
...
#acf part
      ...
      acf(tmp.acf[,2],90,main=paste("自己相関係数\n",colnames(tmp.acf)[2],"\n",date.f,"-",date.e))
      dev.off()
      if(ppp==1 && iii==1){  
        cat(paste("<img src=\"http://archive.am-consulting.co.jp/",png.file,add.ver,"\"", " alt=\"\" width=\"100%\">\n",sep=""),file=output4.html,append=F)
      }else{
