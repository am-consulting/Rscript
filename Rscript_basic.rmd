...
add.ver<-paste("?var=",year(Sys.Date()),month(Sys.Date()),day(Sys.Date()),sep="")
#list
dataset<-list()
dataset.yy<-list()
dataset.xts<-list()
...
if(exe==1){
  ts.source<-list()
  ts.source[[1]]<-c("FRED",1,"EXJPUS")# DCOILWTICO CPIAUCSL NIKKEI225 UNRATE CPIAUCSL GOLDAMGBD228NLBM DEXUSEU DEXUSUK DEXCAUS DEXBZUS DEXUSAL DEXMXUS DEXSFUS DEXUSNZ DEXJPUS SP500 HOUSTNSA DGS10
...
cat(paste("\n$('#",graph.id,"').html('<pre>' + data + '</pre>');",sep=""),file=js.file.name,append=T)
...
for(ppp in 1:plot.type){
  for(iii in 1:length(obj)){
    for(ttt in ttt.range){
      ...
      if(nrow(tmp.1)!=0){
        graph.title<-gsub("\\.億円","",colnames(tmp.1)[2])
        switch(ppp,
...
cat(paste("\n$('#",graph.id,"').html('<pre>' + data + '</pre>');",sep=""),file=js.file.name.3,append=T)
...
#titleはboxplotグラフ内のグラフタイトルと単位根検定結果内のタイトルとしてのみ利用    
    title<-colnames(tmp.1)[2]
    if(regexpr("前年比",colnames(tmp.1)[2])!=-1){
      title<-gsub("億円","パーセント",title, perl=T)
    }
    title<-gsub("\\.億円","",title, perl=T)
    title<-gsub("前年比.","前年比<br>",title, perl=T)
    title<-gsub("\\.うち\\.","<br>うち\\.",title, perl=T)
    title<-gsub("<br>","\\.",title, perl=T)
    if(monthly==1){
      title<-paste(format(tmp.1[1,1],"%Y-%m"),"-",format(tmp.1[nrow(tmp.1),1],"%Y-%m"),"\n",title)
    }else{
      title<-paste(tmp.1[1,1],"-",tmp.1[nrow(tmp.1),1],"\n",title)
    }
#titleはboxplotグラフ内のグラフタイトルと単位根検定結果内のタイトルとしてのみ利用      
#csv、ボックスプロット、単位根共通パート    
...
#cointegration test
setwd(path03)
js.title<-"usdjpynikkei" # mbms mbusdjpy
priod.y<-10 #year
...
chart.TimeSeries(objectDataset.xts[,(explanatory-1)],main=paste(colnames(objectDataset)[explanatory],"×",colnames(objectDataset)[objective],":Level\n",maintitle.first,"-",maintitle.last),yaxis.right=F,type="l",lwd="2",legend.loc="bottomleft",color="blue",cex.main=6,cex.legend=3,cex.lab=2,cex.axis=3,ylab="") 
par(new=T)      
chart.TimeSeries(objectDataset.xts[,(objective-1)],main="",type="l",lwd="2",color="red",xaxis=F,yaxis.right=T,legend.loc="topright",cex.legend=3,ylab="",cex.axis=3)
dev.off()
cat(paste("<img src=\"http://archive.am-consulting.co.jp/",png.file,add.ver,"\"", " alt=\"\" width=\"100%\">\n",sep=""),file=cointeg.file,append=T)
###1stdiff
