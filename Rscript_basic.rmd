...
for(ddd in 1:cnt){
 if(plot.type[[ddd]]==2){
  ...
  if(yy.type[[ddd]]<=nrow(tmp)){
  tmp<-round(diff(tmp,lag=yy.type[[ddd]])/lag(tmp,yy.type[[ddd]])*100,2)[-1]
...
#  if(ddd==1){all.data.xts.yy[[1]]<-tmp}else{all.data.xts.yy[[1]]<-merge.xts(all.data.xts.yy[[1]],tmp,all=T)}
  all.data.xts.yy[[1]]<-merge.xts(all.data.xts.yy[[1]],tmp,all=T)
 ...
 if(length(dir(folder.path))!=0){
  for(fff in 1:length(dir(folder.path))){
   ...
   for(zzz in cnt.start:cnt){
    ...
    plot.type[[zzz]]<-as.numeric(substr(js.title[[zzz]],nchar(js.title[[zzz]]),nchar(js.title[[zzz]])))
...
if(monthly==1){
 data.lag<-12*7#オリジナルデータの長さに注意
 tmp.seasonplot<-tail(tmp.seasonplot,data.lag)  
}else{
 data.lag<-as.Date(paste(year(Sys.Date())-7,"-",month(Sys.Date()),"-1",sep=""))#データの最終日付からでは無いことに注意
