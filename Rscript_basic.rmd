...
if(exe==1){
  ts.source<-list()
  ts.source[[1]]<-c("FRED",1,"NIKKEI225")# DCOILWTICO CPIAUCSL NIKKEI225 UNRATE CPIAUCSL GOLDAMGBD228NLBM DEXUSEU DEXUSUK DEXCAUS DEXBZUS DEXUSAL DEXMXUS DEXSFUS DEXUSNZ DEXJPUS
  ...
  for(iii in 1:length(obj)){
    for(ttt in ttt.range){
      ...
      if(nrow(tmp.1)!=0){
        ...
        setwd(path03)
        png(file=paste(graph.title,".png",sep=""),width=1400,height=800)  
          g<-ggplot()
...
#geom_bar:データが全てプラス値の場合、表示されない
          ...
          g<-g+ggtitle(paste(f.date,"-",e.date))+xlab("")+ylab("")
          g<-g+ylim(y0,y1)
...
for(ppp in 1:plot.type){
  for(iii in 1:length(obj)){
    ...
    colnames(tmp.table)[2]<-gsub("DCOILWTICO","WTI原油価格",colnames(tmp.table)[2], perl=T)  
    if(ttt<=2){
...
#output csv file
    ...
    graph.title<-gsub("WTI原油価格","DCOILWTICO",graph.title, perl=T)  
    graph.title<-gsub("経常収支","CurrentAccountNet",graph.title, perl=T)
    graph.title<-gsub("貿易収支","Goods",graph.title, perl=T)
    graph.title<-gsub("ネット","Net",graph.title, perl=T)
    graph.title<-gsub("輸出","Exports",graph.title, perl=T)
    ...
    graph.title<-gsub(".マネーストック","MoneyStock",graph.title, perl=T)
    graph.title<-gsub("平","",graph.title, perl=T)
    write.csv(tmp.table,paste(gsub(" ", "",graph.title),".csv",sep=""),col.names=T,row.names=F,append=F,quote=F,fileEncoding="UTF-8")
#unit root test      
##text
...
##graph
    ...
    png(file=paste("unitroot",graph.title,".png",sep=""),width=1400,height=800)   
      if(monthly==1){
      ...
      chart.TimeSeries(tmp.unitroot.xts,main=paste(maintitle.first,"-",maintitle.last),yaxis.right=F,type="l",lwd="2",legend.loc="bottomleft",color="blue",cex.main=6,cex.legend=3,cex.lab=2) 
      par(new=T)      
...
#boxplot
    ...
    png(file=paste("boxplot",graph.title,buf,".png",sep=""),width=1400,height=800)   
    ...
    g<-g+geom_boxplot()
    g<-g+ggtitle(tmp.table.title)+xlab("")+ylab("")
