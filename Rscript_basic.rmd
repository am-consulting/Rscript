...
#unit root test js    
    cat("\n\n$(function() {",file=js.file.name,append=T)
    cat(paste("\n$.get('http://archive.am-consulting.co.jp/",result.unitroot.file.name,add.ver,"', function(data) {",sep=""),file=js.file.name,append=T)
...
#unit root test graphimage
    ...
    png(file=unitroot.png,width=1400,height=1200)
    par(mar=c(5, 4, 14, 2))
      if(monthly==1){
...
#unit root test html
    ...
    cat(paste("<img src=\"http://archive.am-consulting.co.jp/",unitroot.png,add.ver,"\"", " alt=\"\" width=\"100%\">\n",sep=""),file=output4.html,append=T)   
...
#boxplot graphimage
    for(ttt in ttt.range){    
      ...
      if(monthly==1){
        tmp.table.title1<-paste(format(tmp.1[1,1],"%Y-%m"),"-",format(tmp.1[nrow(tmp.1),1],"%Y-%m"),"\n",tmp.table.title)
      }else{
