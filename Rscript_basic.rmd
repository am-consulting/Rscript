...
fun.individual<-function(tmp,monthly){
  for(ccc in 2:ncol(tmp)){
    ...
    if(monthly==1){#ソースデータの日付が月末の場合に対処
      tmp.ymd<-paste(year(dataset[[cnt]][,1]),"-",month(dataset[[cnt]][,1]+1),"-1",sep="")
      tmp.ymd.1<-as.Date(tmp.ymd)-1
      dataset[[cnt]][,1]<-tmp.ymd.1
    }
...
if(exe==1){
  ts.source<-list()
  ts.source[[1]]<-c("FRED",1,"HOUSTNSA")# DCOILWTICO CPIAUCSL NIKKEI225 UNRATE CPIAUCSL GOLDAMGBD228NLBM DEXUSEU DEXUSUK DEXCAUS DEXBZUS DEXUSAL DEXMXUS DEXSFUS DEXUSNZ DEXJPUS SP500 HOUSTNSA
...
if(plot.type==2){
  for(ddd in 1:cnt){
    ...
    if(yy.type<=nrow(tmp)){
      ...
      colnames(dataset.xts.yy[[ddd]])<-paste("yearToyear.",colnames(dataset.xts.yy[[ddd]]),sep="")
      ...
      colnames(all.data.xts.yy[[1]])<-paste("yearToyear.",colnames(all.data.xts.yy[[1]]),sep="")
    }
  }
...
#titleはboxplotグラフ内のグラフタイトルと単位根検定結果内のタイトルとしてのみ利用    
    ...
    if(level.multi==-4){
      title<-gsub("億円","兆円",title, perl=T)
    }
    title<-gsub("GOLDAMGBD228NLBM","ゴールド価格.米ドル/トロイオンス",title, perl=T)
    title<-gsub("DCOILWTICO","WTI原油価格.米ドル/バレル",title, perl=T)
...
#output png
png(file=paste("cointeg.",graph.title1,graph.title2,".png",sep=""),width=1400,height=1200) 
par(ps=30)
