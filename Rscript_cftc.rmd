...
#output png
    ...
    chart.TimeSeries(tmp.0[,1],main=paste(currency[iii],"\n",
      ...
      legend.loc="bottomleft",color="black",cex.main=4,cex.legend=3,cex.lab=2)
    par(new=T)
    chart.TimeSeries(tmp.0[,2],main="",type="o",lwd="2",color="blue",xaxis=F,yaxis.right=T,legend.loc="topright",cex.legend=3,ylab="")
    dev.off()
#output html
    ...
    cat(paste("ペア1:",gsub(".FX...CHICAGO.MERCANTILE.EXCHANGE", "",colnames(tmp)[1]),"\nペア2:",colnames(tmp)[2],sep=""),file=result.corr.file.name,append=T)     
    cat(paste("\n相関係数:",cor(tmp)[1,2],sep=""),file=result.corr.file.name,append=T)     
    cat(paste("\np値:",summary(lm(tmp[,2]~tmp[,1]))$coe[8],sep=""),file=result.corr.file.name,append=T)  
    if(iii==1){
      cat(paste("\n<div id=\"","corre",gsub("\\s","", currency[iii], perl=T),"\"></div>",sep=""),file=output2.html,append=F)  
    }else{
