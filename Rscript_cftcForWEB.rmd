...
for(ccc in 1:length(currency)){
  ...
  colnames(dataset[[cnt]])[2:4]<-paste(colnames(dataset[[cnt]])[2:4],":",currency[ccc],sep="")
...
########## 以上、個々のチャートデータを識別されるためにplotData[[1]],plotData[[2]]..plotData[[6]],g1,g2..g6としているが冗長に過ぎるため改良すること      
  ...
  mainTitle<-paste(currency[ccc],":",unit[ccc]," , Period:",first(tmp[,1]),"-",last(tmp[,1]),
    "\nThe upper row:Level , The lower row:1st difference , value:Contracts , Data Source:U.S.Commodity Futures Trading Commission")
