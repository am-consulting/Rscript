...
colnames(mapData)[ncol(mapData)]<-"value"
...
t<-vector()
t<-NULL
interval<-floor((max(mapData$value)-min(mapData$value))/uuu)
for(ttt in 1:uuu){
  t<-c(t,interval*ttt)
}
mapData$classfied<-(findInterval(mapData$value,t)+1)*interval
if(6<=length(unique(mapData$classfied))){break}
}  
q<-ggplot(mapData)
q<-q+aes(long,lat,group=group)+geom_polygon(aes(fill=factor(classfied)))+geom_path(color="white")+coord_equal()
########## patern brewer
if(brew==1){
choroplethColor<-brewer.pal(uuu,"Blues")
q<-q+scale_fill_manual("Population\n",values=choroplethColor,guide="legend")+geom_path(color="white")+coord_equal()
}  
########## patern brewer
...
q<-q+theme(title=element_text(size=40,face="bold"),legend.position="right")   
