```{r}
requiredPackageList<-c("ggplot2","lubridate","scales","maps","maptools","RColorBrewer","fields","rworldmap","classInt","plotrix","HH","reshape","gridExtra","XLConnect","eeptools")
...
fun.readData<-function(){
  ...
  #comma削除
  tmp<-apply(dataset[,-1,drop=F],2,decomma)
  dataset<-cbind(dataset[,1,drop=F],tmp)
  #comma削除
  #ファイル名禁則文字対策
  #禁則文字(ファイル名,フォルダー名)
  #半角記号の \　/　:　*　?　"　<　>　|
...
consumptionTax<-0
perpbr<-0
outputFolder<-1 #1:R_Data_Write,2:charts,3:weeklyResult
autoChartType<-0
needCSV<-1
...
```{r}
#csv出力する際、列名(=個々のファイル名)には少なくとも半角の"," , "%" , "$"は利用しないこと。必ず全角とする。git上で文字化けが発生するため。
fun.readData()
gitURL<-list()
origorbuf<-1 #1 orig , 2 buf
chartSelectNo<-c(1,2,3)
annotationPosition<-0.99
imfPCP<-0
...
mainTitle<-"全産業活動指数 - "
mainTitle<-"鉱工業生産指数前月比 - "
mainTitle<-"軽四輪車新車販売台数 - "
mainTitle<-"住宅着工統計 - "
mainTitle<-"建築着工統計 - "
mainTitle<-"建設工事費デフレーター - "
mainTitle<-"日銀短観 - "
mainTitle<-"米国の時間当たり賃金 - "
mainTitle<-"日本の景気動向指数 - "
mainTitle<-"被保護者調査 - "
mainTitle<-"日本の経常収支と貿易収支 - "
mainTitle<-"日本の特定サービス産業動態売上高指数 - 前年同月比(％)"
mainTitle<-"日本のマネーストック - "
mainTitle<-"日本の国内企業物価指数 - "
mainTitle<-"日本の景気動向指数 - 一致系列 - "
mainTitle<-"日本の労働力調査 - "
mainTitle<-"日本の有効求人倍率 - "
mainTitle<-"週間石油在庫統計 - "
mainTitle<-"日銀金融調整(億円)-"
...
#チャートタイプ選択パート
if(autoChartType==0){
if(nrow(subset(dataset,dataset[,ccc]<0))==0 | nrow(subset(dataset,dataset[,ccc]>0))==0){chartType<-1}else{chartType<-2} 
...
#1原数値 2第二次安倍政権 32014年増税  
tmpbuf.dataset<-buf.dataset
#for(chartNo in 1:3){ 
for(chartNo in chartSelectNo){ 
switch(chartNo,  
  buf.dataset<-buf.dataset,
  buf.dataset<-subset(buf.dataset,as.Date("2013/1/1")<=buf.dataset[,1]),
  buf.dataset<-subset(buf.dataset,as.Date("2014/4/1")<=buf.dataset[,1])
)
#正規化
titleAdd<-""
typeExplanation<-""  
if(chartNo!=1){
if(chartNo==2){titleAdd<-"\n第二次安倍政権発足時"}  
if(chartNo==3){titleAdd<-"\n消費税増税(5%→8%)時"}  
if(chartType==1){
  buf.dataset<-data.frame(Date=buf.dataset[,1],apply(buf.dataset[,-1,drop=F],2,function(x) x/head(x,1)))
  typeExplanation<-"を1とした比"
}else{
  buf.dataset<-data.frame(Date=buf.dataset[,1],apply(buf.dataset[,-1,drop=F],2,function(x) x-head(x,1)))
  typeExplanation<-"を0とした差"
}
colnames(buf.dataset)<-colnames(tmpbuf.dataset)  
}  
#正規化
...
value.y<-max(buf.dataset[,2])
#1989年4月消費税  
if(dateType!=4){taxDate1<-"1989-04-01"}else{taxDate1<-"1989-01-01"}  
if(buf.dataset[1,1]<=as.Date(taxDate1)){
  g<-g+geom_vline(xintercept = as.numeric(as.Date(taxDate1)),linetype=1,colour="#d2691e",size=1)
  g<-g+geom_text(aes(x=as.Date(taxDate1), y=value.y*annotationPosition, label="1989年4月\n消費税導入\n(3%)"), size=8,family="Meiryo",colour="black") 
}
#1989年4月消費税  
#1997年4月消費税  
if(dateType!=4){taxDate2<-"1997-04-01"}else{taxDate2<-"1997-01-01"}  
if(buf.dataset[1,1]<=as.Date(taxDate2)){
  g<-g+geom_vline(xintercept = as.numeric(as.Date(taxDate2)),linetype=1,colour="#d2691e",size=1)
  g<-g+geom_text(aes(x=as.Date(taxDate2), y=value.y*annotationPosition, label="1997年4月\n消費税導入\n(3%→5%)"), size=8,family="Meiryo",colour="black") 
}
#1997年4月消費税  
#2014年4月消費税  
...
if(buf.dataset[1,1]<=as.Date(taxDate3) & tail(buf.dataset[,1],1)>=as.Date(taxDate3)){
  g<-g+geom_vline(xintercept = as.numeric(as.Date(taxDate3)),linetype=1,colour="#d2691e",size=1)
  g<-g+geom_text(aes(x=as.Date(taxDate3), y=value.y*annotationPosition, label="2014年4月\n消費税導入\n(5%→8%)"), size=8,family="Meiryo",colour="black") 
}
#2014年4月消費税  
# #第二次安倍政権発足  
...
#   g<-g+geom_text(aes(x=as.Date(taxDate4), y=value.y*0.2, label="2013年1月\n(2012/12/26)\n第二次安倍政権\n発足"), size=8,family="Meiryo",colour="black") 
# }
# #第二次安倍政権発足  
# #民主党政権発足  
...
#   g<-g+geom_text(aes(x=as.Date(taxDate5), y=value.y*0.2, label="2009年9月\n(2009/9/16)\n民主党政権\n発足"), size=8,family="Meiryo",colour="black") 
# }
# #民主党政権発足  
}  
#消費税パート  
...
maxResult<-tail(maxResult,1)  
minResult<-tail(minResult,1)  
...
totalSentence<-paste(sentence,maxSentence,minSentence,sep="")  
cat(totalSentence,file=weeklyText,append=T) 
}
}  
#weekly summary
if(chartNo==1){  
...
if(chartNo!=1){  
if(chartType==1){  
g<-g+geom_hline(yintercept = as.integer(1),linetype=1,colour="green",size=1)
}else{
g<-g+geom_hline(yintercept = as.integer(0),linetype=1,colour="green",size=1)
}
}  
print(g)
dev.off()
}  
buf.dataset<-tmpbuf.dataset
#1原数値 2第二次安倍政権 32014年増税  
#CSV part
if(needCSV==1){
...
#前週差等をcsv出力  
csvFile<-gsub("\\s","",paste(mainTitle,colnames(buf.dataset)[2]))
csvFile<-paste(csvFile,".csv",sep="")
write.csv(tail(buf.dataset,csvTail),csvFile,row.names=F,append=F,quote=F,fileEncoding="UTF-8")
#前週差等をcsv出力  
}
#link to git part  
...
for(ccc in 2:ncol(dataset)){
cat(paste("- <a href=\"",gitURL[[ccc]],"\">",mainTitle,colnames(dataset)[ccc],"</a><br>\n",sep=""),file=onlyLink,append=T)
}
cat("\n\n\n",file=onlyLink,append=T)
}
#link to git part  
...
cat("<style>.youtube_channel.ytc_wall_1-6 .ytc_video_container {padding: 5px;box-sizing: border-box;font-size: 15px;}</style>
<hr>
...
cat(paste("<hr><div style=\"text-align:center\"><b>第二次安倍政権発足以降の各種指標 - 発足日(月)",typeExplanation,"</b></div>\n",sep=""),file=htmlFile,append=T)
