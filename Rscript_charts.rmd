```{r}
requiredPackageList<-c("ggplot2","lubridate","scales","maps","maptools","RColorBrewer","fields","rworldmap","classInt","plotrix","HH","reshape","gridExtra")
...
setwd(pathOutput)
buf.dataset<-tail(dataset[,c(1,ccc)],tailNumber)  
...
g<-g+ggtitle(paste(mainTitle,"対象期間:",format(head(buf.dataset[,1],1),dateFormat),"-",format(tail(buf.dataset[,1],1),dateFormat),dataSource))
...
g<-g+annotation_custom(
  ...
  xmin=as.numeric(buf.dataset[10,1]),
  xmax=as.numeric(buf.dataset[floor(nrow(buf.dataset)/2),1]),
...
g<-g+annotation_custom(
  ...
  xmin=as.numeric(buf.dataset[floor(nrow(buf.dataset)/2),1]),
  xmax=as.numeric(buf.dataset[floor(nrow(buf.dataset)),1]),
  ymin=min(na.omit(buf.dataset)[,2]),
  ymax=max(na.omit(buf.dataset)[,2])
)
print(g)
