...
mainTitle<-"日本の機械受注統計調査(季節調整値、億円) - "
dataSource<-"\nData Source:国土交通省"
dataSource<-"\nData Source:経済産業省"
#dataSource<-"\nData Source:厚生労働省"
...
if(buf.dataset[1,1]<=as.Date("1989-04-01")){
  g<-g+geom_vline(xintercept = as.numeric(as.Date("1989-04-01")),linetype=1,colour="#d2691e",size=2)
  g<-g+geom_text(aes(x=as.Date("1989-04-01"), y=value.y*0.9, label="1989年4月\n消費税導入(3%)"), size=8,family="Meiryo",colour="black") 
}
if(buf.dataset[1,1]<=as.Date("1997-04-01")){
  g<-g+geom_vline(xintercept = as.numeric(as.Date("1997-04-01")),linetype=1,colour="#d2691e",size=2)
  g<-g+geom_text(aes(x=as.Date("1997-04-01"), y=value.y*0.9, label="1997年4月\n消費税増税(3%→5%)"), size=8,family="Meiryo",colour="black") 
}
if(buf.dataset[1,1]<=as.Date("2014-04-01")){
  g<-g+geom_vline(xintercept = as.numeric(as.Date("2014-04-01")),linetype=1,colour="#d2691e",size=2)
  g<-g+geom_text(aes(x=as.Date("2014-04-01"), y=value.y*0.9, label="2014年4月\n消費税増税(5%→8%)"), size=8,family="Meiryo",colour="black") 
}
if(buf.dataset[1,1]<=as.Date("2013-01-01")){
  g<-g+geom_vline(xintercept = as.numeric(as.Date("2013-01-01")),linetype=1,colour="#d2691e",size=2)
  g<-g+geom_text(aes(x=as.Date("2013-01-01"), y=value.y*0.7, label="2013年1月(2012/12/26)\n第二次安倍政権発足"), size=8,family="Meiryo",colour="black") 
}
if(buf.dataset[1,1]<=as.Date("2009-09-01")){
  g<-g+geom_vline(xintercept = as.numeric(as.Date("2009-09-01")),linetype=1,colour="#d2691e",size=2)
  g<-g+geom_text(aes(x=as.Date("2009-09-01"), y=value.y*0.7, label="2009年9月(2009/9/16)\n民主党政権発足"), size=8,family="Meiryo",colour="black") 
}
}  
#消費税パート  
...
setwd(dir.list)
csvFile<-paste(colnames(dataset[,c(1,ccc)])[2],".csv",sep="")    
write.csv(tail(dataset[,c(1,ccc)],30),csvFile,row.names=F,append=F,quote=F,fileEncoding="UTF-8")
}  
}
cat("<script>\n",file=htmlFile,append=T)
