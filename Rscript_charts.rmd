...
for(ttt in 1:length(successiveCabinets)){
head(successiveCabinets[[ttt]])
tail(successiveCabinets[[ttt]])
}  
#PMパート
```
...
setwd(dir.list)
csvFile<-gsub("\\s","",colnames(dataset[,c(1,ccc)])[2])
...
#CSV part
#link to git part  
gitURL[[ccc]]<-paste("https://raw.githubusercontent.com/am-consulting/CSVDataAtGitHub/master/",csvFile,"<br>\n",sep="")  
#link to git part  
}
setwd(pathOutput)
#link to git part
cat(paste("\n\n<br><b>Link to csv file</b><br>",sep=""),file=htmlFile,append=T)
cat(paste(dataSource,"<br>\n",sep=""),file=htmlFile,append=T)
for(ccc in 2:ncol(dataset)){
cat(gitURL[[ccc]],file=htmlFile,append=T)
}
cat("\n\n\n",file=htmlFile,append=T)
#link to git part  
cat("<script>\n",file=htmlFile,append=T)
