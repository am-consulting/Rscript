...
for(zzz in 2:length(dataset)){
  buf.dataset<-tail(dataset[,c(1,zzz)],tailNumber)  
  SD<-Sys.Date()
  ST<-Sys.time()  
  ...
  switch(chartType,  
    ...
    g<-g+geom_bar(data=buf.dataset,aes(x=xvalue,y=yvalue),stat="identity",position="identity",fill="blue",alpha=0.1,color="black")
  )
  switch(fitType,
    g<-g+geom_smooth(data=buf.dataset,aes(x=xvalue,y=yvalue),method=lm,lwd=2),
    g<-g+geom_smooth(data=buf.dataset,aes(x=xvalue,y=yvalue,group=1),method=loess,color="red",lwd=2)
  )
  ...
  g<-g+theme(axis.text.x=element_text(size=20,angle=90,hjust=1,vjust=0.5),axis.text.y=element_text(size=20)) 
...
titleCol<-1 
setwd(pathOutput)
orderType<-1 # 1:as is , 2-:colnumber
...
if(ncol(buf.dataset)<3){
  colnames(buf.dataset)<-c("xvalue","yvalue")
  g<-ggplot()
  g<-g+geom_bar(data=buf.dataset,aes(x=xvalue,y=yvalue),stat="identity",position="identity",fill="blue",alpha=0.2)
  g<-g+geom_text(data=buf.dataset,aes(x=xvalue,y=yvalue,label=yvalue),hjust=0,size=5,family="Meiryo")
}else{
  ...
  g<-g+geom_text(data=melt.dataset,aes(label=value),hjust=-0.5,size=3,family="Meiryo",position=position_dodge(width=0.9))
}  
g<-g+theme(plot.title=element_text(size=15,family="Meiryo"))
g<-g+theme(axis.title.x=element_text(size=10),axis.title.y=element_text(size=10)) 
g<-g+theme(axis.text.x=element_text(size=20,angle=0,hjust=1,vjust=0.5),axis.text.y=element_text(size=13)) 
...
color=NULL)                                 # 塗り分けに使用する色
{
  map0 <- function(data, dens, color)       # 描画関数
  {
    ...
    main=paste(mainTitle,"\n",creationDate),cex.main="1.0",cex.sub="0.5",family="Meiryo",sub="Choropleth map Powered by http://aoki2.si.gunma-u.ac.jp/ - Script:map.R") # This code is added by AMCC.
    start <- 1
    k <- 0
    ...
    if(textPlot==1){
      text(lon.center,lat.center,round(plotValue,2),cex=textSize,col="black",font=2) # This code is added by AMCC.         
    }
