...
consumptionTax<-1
if(imfPCP==0){
dateType<-4 #1:日次 2:月次 3:character(第*週他) 4:年次
#mainTitle<-"IMF Primary Commodity Prices - "
#mainTitle<-"預金・貸出合計 - "
...
if(buf.dataset[1,1]<=as.Date(taxDate1)){
  ...
  g<-g+geom_text(aes(x=as.Date(taxDate1), y=value.y*0.9, label="1989年4月\n消費税導入\n(3%)"), size=8,family="Meiryo",colour="black") 
}
#1989年4月消費税  
#1997年4月消費税  
...
if(buf.dataset[1,1]<=as.Date(taxDate2)){
  ...
  g<-g+geom_text(aes(x=as.Date(taxDate2), y=value.y*0.9, label="1997年4月\n消費税導入\n(3%→5%)"), size=8,family="Meiryo",colour="black") 
}
#1997年4月消費税  
#2014年4月消費税  
...
if(buf.dataset[1,1]<=as.Date(taxDate5)){
  ...
  g<-g+geom_text(aes(x=as.Date(taxDate5), y=value.y*1.3, label="2009年9月\n(2009/9/16)\n民主党政権発足"), size=8,family="Meiryo",colour="black") 
}
#民主党政権発足  
}  
#消費税パート  
