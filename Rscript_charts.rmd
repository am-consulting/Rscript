...
color=NULL)                                 # 塗り分けに使用する色
{
  map0 <- function(data, dens, color)       # 描画関数
  {
    ...
    main=paste(mainTitle,"\n",creationDate),cex.main="1.0",cex.sub="0.5",family="Meiryo",sub="Choropleth map Powered by http://aoki2.si.gunma-u.ac.jp/") # This code is added by AMCC.
    start <- 1
    k <- 0
    ...
    text(lon.center,lat.center,plotValue,cex=0.5,col="black",font=2) # This code is added by AMCC.         
...
# http://www.inside-r.org/packages/cran/classInt/docs/classIntervals    
    for(zzz in 1:length(classStyle)){
      t<-round((classIntervals(tmp.0[,iii],style=classStyle[zzz],n=5))$brks,2)
      t<-t[-1]
      t<-t[-length(t)]    
      color.set<-brewer.pal(length(t)+1,"Oranges")
      tmp.mainTitle<-mainTitle
      mainTitle<-paste(mainTitle,", 分類方法:",classStyle[zzz])
