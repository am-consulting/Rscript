...
tmp<-readWorksheetFromFile(filename,1,header=F)
dataset<-tmp[-(1:4),]
...
if(dateType!=3){#Date
  ...
  g<-g+ggtitle(paste(
    ...
    dataSource,
    ". Max/Minの対象は",format(dataset[1,1],dateFormat),"以降"
    )
  )
...
cat(paste("<div style=\"text-align:center\"><b>",colnames(buf.dataset)[2],"</b></div>",sep=""),file=htmlFile,append=T)  
