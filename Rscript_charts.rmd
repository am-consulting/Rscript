...
```{r}
########################################## time series ##########################################
mainTitle<-"購買力平価:米ドル-日本円為替レート"
dataSource<-"\nData Source:公益財団法人国際通貨研究所"
...
switch(chartType,  
  ...
  g<-g+geom_bar(data=melt.dataset,aes(x=Date,y=value,color=variable),stat="identity",position="identity",fill="blue",alpha=0.1,color="black")
)
switch(fitType,
  g<-g+geom_smooth(data=melt.dataset,aes(x=Date,y=value),method=lm,lwd=2),
  g<-g+geom_smooth(data=melt.dataset,aes(x=Date,y=value,group=1,color=variable),method=loess,lwd=2),
  g<-g
)
...
g<-g+ggtitle(paste(mainTitle,"\n対象期間:",format(head(buf.dataset[,1],1),"%Y/%m"),"-",format(tail(buf.dataset[,1],1),"%Y/%m"),dataSource))
...
g<-g+geom_text(aes(x=tail(buf.dataset[,1],1),y=as.numeric(tail(buf.dataset[,-1],1)),
  label=paste(format(tail(buf.dataset[,1],1),"%Y/%m"),":",as.numeric(tail(buf.dataset[,-1],1)))),size=10,hjust=1)
print(g)
dev.off()
########################################## time series ##########################################
```
...
```{r}
##########################################  bar chart  ##########################################
grandTitle<-"業種別PER.PBR"
dataSource<-"\nData Source:株式会社日本取引所グループ"
fun.readData()
colorList<-c("blue","red") #改良余地あり
titleCol<-2 
setwd(pathOutput)
orderType<-2 # 1:as is , 2-:colnumber
...
if(ncol(buf.dataset)<3){
  ...
  g<-g+geom_text(data=buf.dataset,aes(x=xvalue,y=yvalue,label=yvalue),hjust=0,size=7,family="Meiryo")
  ...
  g<-g+theme(legend.title=element_text(colour="white",size=0,face="bold"))   
  g<-g+scale_fill_manual(values=colorList) 
  g<-g+geom_text(data=melt.dataset,aes(label=value),hjust=-0.5,size=4,family="Meiryo",position=position_dodge(width=0.9))
}  
g<-g+theme(plot.title=element_text(size=40,family="Meiryo"))
g<-g+theme(axis.title.x=element_text(size=30)) 
g<-g+theme(axis.title.y=element_text(size=30)) 
