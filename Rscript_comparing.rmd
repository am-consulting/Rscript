...
plotText1<-NULL
originalSeries<-(tail(allData[,2],-12)-head(allData[,2],-12))/head(allData[,2],-12)*100
result<-capture.output(adf.test(originalSeries))
for(ttt in 1:length(result)){plotText1<-paste(plotText1,result[ttt],"\n")}
y0<-min(originalSeries)
y1<-max(originalSeries)
...
g1<-ggplot()
g1<-g1+scale_y_continuous(labels=comma,limits=c(y0,y1+(y1-y0)/5))
g1<-g1+geom_line(aes(x=tail(allData[,1],-12),y=originalSeries),size=1)
g1<-g1+geom_smooth(aes(x=tail(allData[,1],-12),y=originalSeries),method=loess,color="red")
g1<-g1+ggtitle(paste(colnames(allData)[2]))
g1<-g1+xlab("") 
g1<-g1+ylab("") 
g1<-g1+theme(axis.text.x=element_text(size=30,face="bold",family="Meiryo"))          
...
#####################################
plotText2<-NULL
originalSeries<-(tail(allData[,3],-12)-head(allData[,3],-12))/head(allData[,3],-12)*100
result<-capture.output(adf.test(originalSeries))
for(ttt in 1:length(result)){plotText2<-paste(plotText2,result[ttt],"\n")}
y0<-min(originalSeries)
y1<-max(originalSeries)
...
g2<-ggplot()
g2<-g2+scale_y_continuous(labels=comma,limits=c(y0,y1+(y1-y0)/5))
g2<-g2+geom_line(aes(x=tail(allData[,1],-12),y=originalSeries),size=1)
g2<-g2+geom_smooth(aes(x=tail(allData[,1],-12),y=originalSeries),method=loess,color="red")
g2<-g2+ggtitle(paste(colnames(allData)[3]))
g2<-g2+xlab("") 
g2<-g2+ylab("") 
g2<-g2+theme(axis.text.x=element_text(size=30,face="bold",family="Meiryo"))          
...
if(ppp==1){
  ...
  title0<-"原系列の相互相関係数(単位根検定結果に注意ください)\n"
}  
title<-paste(
  title0,
  "説明変数:",
...
cat("<script src=\"https://cdn.datatables.net/1.10.7/js/jquery.dataTables.min.js\"></script>",file=htmlFile,append=T) 
