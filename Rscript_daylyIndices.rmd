...
tsSource<-list()
dataset<-list()
tsSource[[1]]<-c("FRED",1,"http://research.stlouisfed.org/","DEXJPUS","DJIA","GOLDAMGBD228NLBM","DCOILWTICO","NIKKEI225","DEXSFUS","DEXUSAL","DEXUSEU","DGS10")
...
#tsSource[[1]]<-c("FRED",1,"http://research.stlouisfed.org/","SP500")
...
dateFormat<-"%Y/%m/%d"
tmp<-na.omit(dataset[[cnt]])
tmp<-data.frame(tmp[-1,1],tmp[-1,2],diff(tmp[,2])/tmp[-1,2]*100)
...
par(ps=20,mar=marType,cex.main=1.5,cex.lab=1.5,cex.axis=1.5,family="Meiryo")   
tmp<-na.omit(dataset[[cnt]])
tmp<-data.frame(tmp[-1,1],diff(tmp[,2])/tmp[-1,2]*100)
