...
#対象は日経×ドル円、ドルインデックス×ゴールド、ドルインデックス×WTI原油、米ドル×日米金利差とする
cat(paste("<div style=\"text-align:center\"><b>正規化時系列チャート</b></div>",sep=""),file=htmlFile,append=T)
cols<-c("black", "blue")
ltys<-c(1,1)
lwds<-c(1,1)
normalData<-list()
normalObj<-c("NIKKEI225","DEXJPUS","GOLDAMGBD228NLBM","DCOILWTICO","DTWEXB")
...
legend("topleft",legend=labels,col=cols,lty=ltys,lwd=lwds,cex=1.5)  
dev.off()
cat(paste("<hr><img src=\"",png.file,"\" alt=\"",png.file,"\" width=\"100%\">",sep=""),file=htmlFile,append=T)
}
#正規化パート
```
