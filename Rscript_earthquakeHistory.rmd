...
columnWidth<-"{\"width\":\"8%\",\"targets\":0},{\"width\":\"16%\",\"targets\":1},{\"width\":\"9%\",\"targets\":2},{\"width\":\"9%\",\"targets\":3},{\"width\":\"9%\",\"targets\":4} ,{\"width\":\"11%\",\"targets\":6}"
switch(dateFormat,
  dateFormatType<-"",
  dateFormatType<-"",
  dateFormatType<-"%Y/%m",
...
magBorder<-8
kmlDataset<-dataset
kmlDataset<-subset(kmlDataset,magBorder<=kmlDataset[,6])
setwd(pathOutput)
addVer<-gsub("\\.","",paste(year(SD),month(SD),day(SD),hour(ST),minute(ST),second(ST),sep=""))
...
cat("-->\n",file=kmlhtml,append=T)
cat(paste("<div style=\"text-align:center\">1900年以降に発生したマグニチュード",magBorder,"以上の発生地震マップ. ソースデータ確認日:",year(SD),"年",month(SD),"月",day(SD),"日. Data Source:<a href=\"http://earthquake.usgs.gov/\" target=\"_blank\">http://earthquake.usgs.gov/</a>","</div>\n",sep=""),file=kmlhtml,append=T)
...
cat("var kmlPath = \"http://archive.am-consulting.co.jp/earthquakesOverMagnitude8afterY1900.kml\";\n",file=kmlhtml,append=T)
