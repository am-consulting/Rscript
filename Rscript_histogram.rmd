...
tsSource[[1]]<-c("FRED",1,"http://research.stlouisfed.org/","NIKKEI225")
...
for(ddd in 1:length(dataset)){
  ...
  buf<-subset(buf,as.Date("1980/1/1")<=buf[,1])
  ...
  binWidth<-10^(floor(log10(range(histgramMonth[[ddd]][,2])[2]-range(histgramMonth[[ddd]][,2])[1]))-2)
  ...
  png.file<-paste(addVer,".png",sep="")
  png(file=png.file,width=1920,height=1080)
  par(oma=c(1,1,10,1),ps=20,mar=c(5,10,10,3),cex.main=2,cex.lab=2,cex.axis=1.5,family="Meiryo",mfrow=c(1,2)) 
  #left part
  ...
  title(paste(title,"\nShapiro-Wilk normality test. p value=",
    shapiro.test(histgramMonth[[ddd]][,2])$p.value),outer=TRUE,cex.main=1.5,family="Meiryo")
  dev.off()
  #chart04
  ST<-Sys.time()  
  addVer<-gsub("\\.","",paste(year(SD),month(SD),day(SD),hour(ST),minute(ST),second(ST),sep=""))
  ...
  png.file<-paste(addVer,".png",sep="")
  png(file=png.file,width=1920,height=1080)
  par(oma=c(1,1,10,1),ps=20,mar=c(5,2,5,1),cex.main=2,cex.lab=2,cex.axis=1.0,family="Meiryo",mfrow=c(1,2)) 
  ...
  title(paste(title),outer=TRUE,cex.main=1.5,family="Meiryo")
  dev.off()
  #####chart part
}
```