...
cat("<b>Speech Recognition APIを利用した簡易分析結果の読み上げ</b><br>\n",file=htmlFile,append=T)
cat(" - 視聴可能ウェブブラウザは http://caniuse.com/#feat=speech-recognition で確認ください。<br>\n",file=htmlFile,append=T)
cat(" - chrome44での動作は確認済みです。\n",file=htmlFile,append=T)
cat("<div style=\"text-align:center\"><INPUT type=\"button\" value=\"簡 易 分 析 結 果 を 聴 く\" onclick=\"speech();\"></div><hr>\n",file=htmlFile,append=T)
...
maxCurrency<-dataset[which.max(dataset[,8]),]
minCurrency<-dataset[which.min(dataset[,8]),]
prePhrase<-list()
...
prePhrase[[3]]="紹介対象通貨は、米ドル、ユーロ、の2通貨です。"
prePhrase[[4]]="米ドル、の"
...
prePhrase[[12]]="対象期間内で、"
prePhrase[[13]]=paste("最も変動係数の大きい通貨は、",maxCurrency[1,1],"、で",maxCurrency[1,8],"パーセント、",sep="")
prePhrase[[14]]=paste("最も変動係数の小さい通貨は、",minCurrency[1,1],"、で",minCurrency[1,8],"パーセント、です。",sep="")
prePhrase[[15]]="紹介は以上です。ご視聴、ありがとうございました。"
