...
for(mmm in floor(min(dataset.j$mag)):floor(max(dataset.j$mag))){
  ...
  g<-g+geom_text(data=tmp,aes(x=longitude, y=latitude,label=mag),size=8,family="Meiryo")
...
for(pppp in 1:2){
  ...
  tmp<-data.frame(paste(dataset.j[,17],"/",dataset.j[,14],sep=""),dataset.j[,objectCol])
