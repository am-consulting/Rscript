...
```{r}
for(mmm in floor(min(datasetMap$mag)):floor(max(datasetMap$mag))){
  earthquakeList<-NULL
  tmp<-subset(datasetMap,mmm<=(datasetMap$mag) & (datasetMap$mag)<(mmm+1))
...
for(pppp in 1:2){
  if(pppp==1){
    mainTitle<-paste(subTitle,date.e,"から",date.f,"\nマグニチュード降順(上位20)\nソースデータ確認日時:",dataTime,sourceURL)
    barColor<-"blue"
    objectCol<-grep("mag",colnames(datasetMap))
    ylabTitle<-"マグニチュード"
  }else{
    mainTitle<-paste(subTitle,date.e,"から",date.f,"\nエネルギー降順(上位20)\nソースデータ確認日時:",dataTime,sourceURL)
    barColor<-"red"  
  ...
  colnames(datasetMap)
  tmp<-data.frame(paste(datasetMap$JST,"\n",datasetMap$place,sep=""),datasetMap[,objectCol],datasetMap$magType)
  ...
  if(pppp==1){  
    g<-g+geom_text(data=tmp,aes(x=tmp[,1],y=tmp[,2],label=paste(tmp[,2],"(",tmp[,3],")",sep="")),vjust=0.5,hjust=2,size=5,face="bold",family="Meiryo") 
  }
  ST<-Sys.time()  
  addVer<-gsub("\\.","",paste(year(SD),month(SD),day(SD),hour(ST),minute(ST),second(ST),sep=""))
  png.file<-paste(addVer,".png",sep="")
