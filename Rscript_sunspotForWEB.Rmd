  ...
  for(iii in 1:(length(url))){
    ...
    if(ppp==1 | ppp==3){
      ...
      if(iii==1){
        ...
        g<-g+geom_line(data=ssnTS,aes(x=date,y=value,colour=variable),alpha=0.5,size=1)
        g<-g+scale_colour_manual("時系列データ",values=c("blue","black"))
        g<-g+geom_point(data=tmp,aes(x=tmp[,1],y=tmp[,3]),colour="black",size=2)
        g<-g+geom_smooth(data=tmp,aes(x=tmp[,1],y=tmp[,2]),method=loess,color="red")
        g<-g+geom_smooth(data=tmp,aes(x=tmp[,1],y=tmp[,3]),method=loess,color="red")
      }else{
        g<-g+geom_line(data=tmp,aes(x=tmp[,1],y=tmp[,2]),colour="blue",alpha=0.5,size=1)
        g<-g+geom_smooth(data=tmp,aes(x=tmp[,1],y=tmp[,2]),method=loess,color="red")
      }  
      g<-g+geom_point(data=tmp,aes(x=tmp[,1],y=tmp[,2]),colour="blue",size=2)
...
for(ppp in 1:(length(url))){
  ...
  png.file<-paste(addVer,".png",sep="")
  png(file=png.file,width=1920,height=1080)
  par(cex.main=3,cex.axis=3,cex.lab=3,cex.sub=4,mar=c(8,8,8,2))
  MainTitle<-paste("自己相関関数-",mainTitle,"-期間:",date.s,"-",date.e,datasourceURL)
