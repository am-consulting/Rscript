...
shinyServer(function(input, output, session)
{
  fun <- function(sss) {
    ...
    if (nrow(tmp) != 0) {
      tmp <-
        subset(tmp,
               input$dataRange[1] <= index(tmp) &
                 index(tmp) <= input$dataRange[2])
      buf <- colnames(tmp)
      if (input$datatype == 2)
        #1st diff
      {
      ...
      assign(paste("plot", sss, sep = ""), renderPlot({
        ...
        if (input$charttype == "l" || input$charttype == "h")
        {
          ...
          #########
          bufmergeplotData <-
            merge(plotData, origData5, by = "Date")
          par(new = T)
          plot(
            bufmergeplotData[, 1],
            bufmergeplotData[, 3],
          ...
          legend(
            ...
            col = "red",
            lty = 1,
            legend = colnames(bufmergeplotData)[3],
            cex = 1
          )
          mtext(
            colnames(bufmergeplotData)[3],
            side = 4,
            line = 3.2,
            cex = 1
          )
  ...
  output$gitcode <- renderUI({
    ...
    <b>Code</b><br>
    <ol>
    <li><a href=\"https://github.com/am-consulting/Rscript/tree/master/shinyapps/InternationalReservesOfJapan\" target=\"_blank\">Move to GitHub</a></li>
    </ol>"
    HTML(str)
  })
