...
shinyServer(function(input, output, session)
{
  fun <- function(sss) {
    ...
    if (nrow(tmp) != 0) {
      tmp <-
        subset(tmp,
               input$dataRange[1] <= index(tmp) & index(tmp) <= input$dataRange[2])
      buf <- colnames(tmp)
      if (input$datatype == 2)
        #1st diff
      {
  ...
  reactiveData <- reactive({
    ...
    output$title01 <-
      output$title001 <- output$title0001 <- get(paste("title", sss, sep = ""))
    ...
    output$title02 <-
      output$title002 <- output$title0002 <- get(paste("title", sss, sep = ""))
    ...
    output$title03 <-
      output$title003 <- output$title0003 <- get(paste("title", sss, sep = ""))
    ...
    output$title04 <-
      output$title004 <- output$title0004 <- get(paste("title", sss, sep = ""))
  })
  ...
  output$gitcode <- renderUI({
    ...
    <b>Code</b><br>
    <ol>
    <li><a href=\"https://github.com/am-consulting/Rscript/tree/master/shinyapps/JapansBalanceofPayments\" target=\"_blank\">Move to GitHub</a></li>
    </ol>"
    HTML(str)
  })
