...
shinyServer(function(input, output, session)
{
  reactiveData <- reactive({
    ...
    for (iii in 1:length(yearList)) {
      yyData[[iii]] <-
        subset(plotData, year(plotData[, 1]) == yearList[iii])
    }
    ...
    seasonData <-
      data.frame(yearmonthDF, month = month.abb[as.numeric(substr(yearmonthDF[, 1], 6, 7))], check.names = F)
    seasonData$month <-
    ...
    output$commodityName <- DT::renderDataTable(
      commodityName,
      rownames = F,
    ...
    funPlot <- function(iii) {
      assign(paste("tsPlot", iii, sep = ""), renderPlot({
        par(mar = c(3, 4, 3, 4))
        switch (
          iii,
...
#    output$plot06 <- get(paste("tsPlot", iii, sep = ""))
  ...
  output$gitcode <- renderUI({
    ...
    <b>Code</b><br>
    <ol>
    <li><a href=\"https://github.com/am-consulting/Rscript/tree/master/shinyapps/commodityAnalysis\" target=\"_blank\">Move to GitHub</a></li>
    </ol>"
    HTML(str)
  })
